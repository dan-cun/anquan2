...
```text
D:\SecMind\security-tools\bin\              # 独立 CLI
D:\SecMind\security-tools\packages\         # 解压的软件包
D:\SecMind\security-mcp\servers\            # 自编或第三方 MCP Server
D:\SecMind\security-mcp\venvs\              # Python 虚拟环境
D:\SecMind\security-mcp\config\             # 非敏感配置
D:\SecMind\security-mcp\logs\               # 运行日志
...
| CLI 路径 | `D:\SecMind\security-tools\bin\nmap.exe` |
| 版本检查 | `nmap --version` |
...
```powershell
py -3.11 -m venv D:\SecMind\security-mcp\venvs\b-security-tools
D:\SecMind\security-mcp\venvs\b-security-tools\Scripts\python.exe -m pip install --upgrade pip
D:\SecMind\security-mcp\venvs\b-security-tools\Scripts\python.exe -m pip install "mcp[cli]>=1.12,<2"
```
...
```powershell
$env:BTOOLS_NMAP='D:\SecMind\security-tools\bin\nmap.exe'
$env:BTOOLS_WORK_ROOT='C:\security-workspaces'
D:\SecMind\security-mcp\venvs\b-security-tools\Scripts\python.exe D:\SecMind\security-mcp\servers\b_security_tools.py
```
...
Windows 推荐使用任务计划程序、WinSW 或已经部署的服务管理平台启动 Python/Node 进程。服务账户必须能够读取 CLI、工作目录和所需环境变量，但不应使用日常管理员账户。配置恢复策略，并将 stdout/stderr 写入 `D:\SecMind\security-mcp\logs`。
...
```json
{
  ...
  "transport": "stdio",
  "command": "D:/SecMind/security-mcp/venvs/tools/Scripts/python.exe",
  "args": ["D:/SecMind/security-mcp/servers/tools.py"],
  "cwd": "D:/SecMind/security-mcp/servers",
  "env_refs": {
